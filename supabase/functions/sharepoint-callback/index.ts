...
Deno.serve(async (req) => {
  try {
    ...
    console.log("SharePoint connected for user:", userId, "Microsoft account:", profile.userPrincipalName || profile.mail);
    ...
    return new Response(
      `<html><body><script>
        window.opener?.postMessage({ type: 'SHAREPOINT_CONNECTED' }, '*');
        window.close();
